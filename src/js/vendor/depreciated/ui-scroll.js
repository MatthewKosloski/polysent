    ...
    return {
      ...
      compile: function(elementTemplate, attr, linker) {
        return function($scope, element, $attr, controllers) {
          ...
          clipBottom = function() {
            var bottomHeight, i, item, itemHeight, itemTop, newRow, overage, rowTop, _ref;
            bottomHeight = 0;
            overage = 0;
            for (i = _ref = buffer.length - 1; _ref <= 0 ? i <= 0 : i >= 0; _ref <= 0 ? i++ : i--) {
              item = buffer[i];
          ...
          adjustBuffer = function(rid, finalize) {
            var i, promises, toBePrepended, toBeRemoved, wrapper, _len;
            promises = [];
            toBePrepended = [];
            toBeRemoved = [];
            for (i = 0, _len = buffer.length; i < _len; i++) {
              wrapper = buffer[i];
              switch (wrapper.op) {
                case 'prepend':
            ...
            return $timeout(function() {
              var i, item, itemHeight, itemTop, newHeight, newRow, rowTop, topHeight, wrapper, _i, _j, _k, _len2, _len3, _len4, _len5;
              for (_i = 0, _len2 = toBePrepended.length; _i < _len2; _i++) {
                wrapper = toBePrepended[_i];
                builder.insertElement(wrapper.element);
              ...
              for (_j = 0, _len3 = toBeRemoved.length; _j < _len3; _j++) {
                wrapper = toBeRemoved[_j];
                promises = promises.concat(removeItem(wrapper));
              }
              for (i = 0, _len4 = buffer.length; i < _len4; i++) {
                item = buffer[i];
              ...
              if (pending.length === 0) {
                topHeight = 0;
                for (_k = 0, _len5 = buffer.length; _k < _len5; _k++) {
                  item = buffer[_k];
                  itemTop = item.element.offset().top;
                  newRow = rowTop !== itemTop;
                  rowTop = itemTop;
          ...
          applyUpdate = function(wrapper, newItems) {
            var i, keepIt, newItem, pos, _len, _ref;
          ...
          adapter.applyUpdates = function(arg1, arg2) {
            var bufferClone, i, wrapper, _len, _ref;
            dismissPendingRequests();
            if (angular.isFunction(arg1)) {
              bufferClone = buffer.slice(0);
              for (i = 0, _len = bufferClone.length; i < _len; i++) {
